...
async def handle_create_player(request: CreatePlayerRequest, redis: Redis):
    ...
    # nx=True only sets if key doesn't exist — returns True if set, False if already taken
    is_unique = await redis.set(f"nickname:{nickname}", "__reserved__", nx=True, ex=3600)
    if not is_unique:
